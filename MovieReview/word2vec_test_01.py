def word2vec_test():
    ...
    def review_to_bagofwords(review):
        from sklearn.feature_extraction.text import CountVectorizer
        wordlist = review_to_words(review)
        vectorizer = CountVectorizer()
        train_feature = vectorizer.fit_transform(wordlist)
        return train_feature.toarray()       
    def get_data_features(datapath):     
        ...
        features = []
        cnt = 0
        for review in dataset['review'][0:10]:
            if( (cnt+1)%1000 == 0 ):
                print cnt
            features.append(review_to_bagofwords(review))#review_to_wordofcluster(review,words_idx)
            cnt +=1
