...
def get_ind_com_sum_table(X,rule):
    ...
    for rii in roi_index_ind:
        roi_score_metrics_file.write(str(index2roi(rii,rule)[0])+'_'+str(index2roi(rii,rule)[1])+'\t'+str(acc_score_ind)+'\t'+str(weight_ind[rii])+'\t'+str(index2roi(rii,rule)[1])+'\t'+str(index2roi(rii,rule)[2])+'\n')
    roi_score_metrics_file.write('---------This is combined one:-------------'+'\n')
    roi_score_metrics_file.write('ROI'+'\t'+'Accuracy'+'\t'+'weight'+'\t'+'Condition'+'\t'+'Brain_Metrics'+'\n')
    for rii in roi_index_com:
        roi_score_metrics_file.write(str(index2roi(rii,rule)[0])+'\t'+str(column_max_com)+'\t'+str(weight_com[roi_index_com.index(rii)])+'\t'+str(conMatrix_ms[end_index])+'\t'+str(bap.bacc_p(conMatrix_ms[end_index]))+'\t'+str(bacc_mean_ms[end_index])+'\t'+str(index2roi(rii,rule)[1])+'\t'+str(index2roi(rii,rule)[2]+'\n'))
